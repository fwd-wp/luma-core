...
final class TemplateTags
{
	...
	public static function posted_on(): void
	{
		...
		printf(
			...
			esc_html__('Published %s', 'twentyone'),
	...
	{ ?>
		<div class="posted-meta">
			<?php self::posted_on(); ?>
			<?php if (!(ThemeMod::get('twenty_one_post_display_author_bio'))): ?>
				<?php self::posted_by(); ?>
			<?php endif; ?>
		...
		if (has_category() || has_tag()) {
			...
			if ($categories_list) {
				printf(
					...
					'<span class="cat-links">' . esc_html__('Categorized as %s', 'twentyone') . ' </span>',
			...
			if ($tags_list && ! is_wp_error($tags_list)) {
				printf(
					...
					'<span class="tags-links">' . esc_html__('Tagged %s', 'twentyone') . '</span>',
...
<?php
		...
		the_posts_pagination(
			array(
				...
				'next_text'          => sprintf(
					...
					wp_kses(
						__('Older <span class="nav-short">posts</span>', 'twentyone'),
						array(
							'span' => array(
								'class' => array(),
							),
	...
	public static function page_links(): void
	{
		wp_link_pages(
			array(
				'before'   => '<nav class="page-links" aria-label="' . esc_attr__('Page', 'twentyone') . '">',
				'after'    => '</nav>',
				/* translators: %: Page number. */
				'pagelink' => esc_html__('Page %', 'twentyone'),
			)
		);
	}
	/**
